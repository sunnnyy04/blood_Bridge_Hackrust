...
const MESSAGING_SERVICE_URL = process.env.MESSAGING_SERVICE_URL || "http://localhost:3001";
bloodApi.post("/request", async (c) => {
    ...
    const registeredDonors = await db
        .select()
        .from(user)
        .where(and(eq(user.role, "donor"), eq(user.isAvailable, true), inArray(user.bloodType, compatibleTypes), requestType === "plasma" ? eq(user.canDonatePlasma, true) : eq(user.canDonateBlood, true), or(isNull(user.lastDonatedAt), lt(user.lastDonatedAt, threeMonthsAgo))));
    const nearbyRegisteredDonors = registeredDonors.filter((d) => {
        if (!d.latitude || !d.longitude)
            return false;
        return (haversineDistance(hospitalLat, hospitalLon, parseFloat(d.latitude), parseFloat(d.longitude)) <= radiusKm);
    });
    let dbCount = 0;
...
bloodApi.get("/requests/:id/responses", async (c) => {
    ...
    const response = await fetch(`${MESSAGING_SERVICE_URL}/requests/${requestId}/responses`);
    const data = await response.json();
    return c.json(data);
});
