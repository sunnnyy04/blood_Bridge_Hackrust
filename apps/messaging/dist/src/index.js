...
import { db, mockDonors, bloodRequests, bloodRequestResponses, } from "./db/schema.js";
...
app.post("/send-alert", async (c) => {
    ...
    const allDonors = await db
        .select()
        .from(mockDonors)
        .where(and(eq(mockDonors.isAvailable, true), inArray(mockDonors.bloodType, compatibleTypes), requestType === "plasma" ? eq(mockDonors.canDonatePlasma, true) : eq(mockDonors.canDonateBlood, true), or(isNull(mockDonors.lastDonatedAt), lt(mockDonors.lastDonatedAt, threeMonthsAgo))));
...
app.post("/webhook/whatsapp", async (c) => {
    ...
    if (response === "yes") {
        await db
            ...
            .where(eq(mockDonors.phone, from));
        const replyMessage = "Thank you for confirming! The hospital will contact you shortly. 🩸";
        if (twilioClient && fromNumber) {
            try {
                const formattedFrom = fromNumber.startsWith("whatsapp:") ? fromNumber : `whatsapp:${fromNumber}`;
                await twilioClient.messages.create({
